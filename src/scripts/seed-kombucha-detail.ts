...
async function seedKombuchaDetail() {
  ...
  try {
    ...
    const mergedEN = {
      ...
      onlineBullets: (dePage as any).workshopDetail?.onlineBullets?.map(
        (item: { id?: string | null }, i: number) => ({
          ...workshopDetailEN.onlineBullets?.[i],
          id: item.id,
        }),
      ),
      genericFaqItems: (dePage as any).workshopDetail?.genericFaqItems?.map(
        (item: { id?: string | null }, i: number) => ({
          ...workshopDetailEN.genericFaqItems?.[i],
          id: item.id,
        }),
      ),
      teamBullets: (dePage as any).workshopDetail?.teamBullets?.map(
        (item: { id?: string | null }, i: number) => ({
          ...workshopDetailEN.teamBullets?.[i],
          id: item.id,
        }),
      ),
      whyOnlineFeatures: (dePage as any).workshopDetail?.whyOnlineFeatures?.map(
        (item: { id?: string | null }, i: number) => ({
