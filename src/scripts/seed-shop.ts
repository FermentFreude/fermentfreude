...
async function seedShop() {
  ...
  const featuredItemsWithIds = (shopDE?.featuredItems ?? []).map(
    (item: { id?: string | null }) => ({
          ...
          ? [
              {
                ...
                productImage: heroProduct1Id ?? heroImageId,
                title: 'LAKTO FERMENTATION.',
                description: 'Discover the art of fermented vegetables and natural preservation.',
                ctaLabel: 'Shop Now',
                ctaUrl: '#products',
              },
              {
    ...
    benefitsItems: [
      {
        ...
        description:
          'Probiotika in fermentierten Lebensmitteln unterstützen die Verdauung und ein ausgewogenes Mikrobiom.',
      },
      {
        title: 'Nährstoff-Boost',
        description:
          'Fermentation erhöht die Bioverfügbarkeit von Vitaminen, Mineralstoffen und Enzymen.',
      },
      {
        title: 'Natürliche Konservierung',
        description:
          'Keine Zusatzstoffe nötig – Fermentation konserviert Lebensmittel natürlich mit lebenden Kulturen.',
      },
    ],
    giftHeading: 'Verschenke ein besonderes Geschmackserlebnis',
    giftDescription:
  ...
  if (existingShopPage.docs.length > 0) {
    ...
    const freshDE = (await payload.findByID({
      ...
      locale: 'de',
      depth: 0,
    })) as { shop?: { featuredItems?: { id?: string }[]; heroSlides?: { id?: string }[] } }
    ...
    const shopPageDataEN = {
      ...
      benefitsItems: [
        ...
        },
        {
          ...
          description: 'Fermentation enhances bioavailability of vitamins, minerals, and enzymes.',
        },
        {
    ...
    payload.logger.info(
      '✓ Shop page in Pages collection updated (DE + EN). Edit at /admin/collections/pages',
    )
  } else {
    ...
    payload.logger.info(
      '✓ Shop page in Pages collection created (DE + EN). Edit at /admin/collections/pages',
    )
  }
