...
export function generateWorkshopReceiptPDF(data: WorkshopReceiptData): Buffer {  const b = data.business ?? {}
  ...
  const titleText =
    data.locale === 'de' ? 'TEILNAHMEBESTÄTIGUNG' : 'PARTICIPATION CONFIRMATION'
  doc.setFont('helvetica', 'bold')
  ...
  // Column 1: Hinweis (this document is your ticket; invoice sent separately)
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(6.5)
  doc.setTextColor(...COLORS.grayLabel)
  ...
  const ticketLines = splitLines(doc, ticketNote, contentWidth / 3 - 2 * footerBoxPad)
