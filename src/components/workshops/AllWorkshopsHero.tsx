...
/* ═══════════════════════════════════════════════════════════════
 *  AllWorkshopsHero — Hero for /workshops (all workshops overview)
 *
 *  Full-viewport split layout matching LaktoHero:
 *  LEFT:  Jar silhouettes (placeholder)
 *  RIGHT: Eyebrow + title + description + attributes
 *
 *  CMS-editable via the Pages collection Hero tab.
...
export type AllWorkshopsHeroCMS = {
  eyebrow?: string | null
  title?: string | null
...
export function AllWorkshopsHero({ cms }: { cms?: AllWorkshopsHeroCMS }) {
  // CMS values with English defaults
  const eyebrow = cms?.eyebrow ?? 'Workshop Experience'
  const titleRaw = cms?.title ?? 'Entdecke die Kunst\nder Fermentation'
  const titleLines = titleRaw.split('\n')
  const description =
    cms?.description ??
    'Lerne von Experten, wie du gesunde fermentierte Lebensmittel selbst zubereitest. Wähle aus vier Workshop-Arten für Anfänger und Fortgeschrittene.'
  const attributes =
