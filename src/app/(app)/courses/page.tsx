...
export default async function CoursesPage({ searchParams }: CoursesPageProps) {
  ...
  // Fetch Basic Fermentation Course product to display price even if CMS card omits it
  const basicCourseProductResult = await payload.find({
    collection: 'products',
    where: { slug: { equals: 'basic-fermentation-course' } },
    limit: 1,
    depth: 0,
  })
  const basicCourseProduct = basicCourseProductResult.docs[0] as Product | undefined
  // Normalize price so it looks right even if value was entered as cents (e.g. 9800 → 98.00 €)
  const basicCourseRawPrice = basicCourseProduct?.priceInEUR
  const basicCourseNormalizedPrice =
    basicCourseRawPrice != null
      ? basicCourseRawPrice > 1000
        ? basicCourseRawPrice / 100
        : basicCourseRawPrice
      : 49
  const basicCoursePrice = new Intl.NumberFormat(locale === 'de' ? 'de-DE' : 'en-US', {
    style: 'currency',
    currency: 'EUR',
  ...
  if (oc) {
    ...
    ;((oc.onlineCoursesWorkshopCards ?? []) as Array<{ image?: unknown }>).forEach((c) =>
      addId(c.image),
    )
    ...
    for (const id of mediaIds) {
      try {
        ...
        if (doc?.id != null) {
          ...
          const rawUrl =
            typeof mediaDoc?.url === 'string' && mediaDoc.url.length > 0 ? mediaDoc.url : null
          const filename =
            typeof mediaDoc?.filename === 'string' && mediaDoc.filename.length > 0
              ? mediaDoc.filename
              : null
          const urlValid =
            rawUrl &&
  ...
  const resolveMedia = (v: unknown): MediaType | null => {
    ...
    const mediaUrl =
      typeof v === 'object' && v !== null && 'url' in v ? (v as MediaType).url : undefined
    if (typeof mediaUrl === 'string' && mediaUrl.length > 0) return v as MediaType
    return fromMap ?? null
  }

  // Fetch home page for TeamPreview block
  ...
  if (!forceDefaults) {
    ...
    teamPreviewBlock = homeBlocks.find((block) => block?.blockType === 'teamPreview') as
      | NonNullable<PageType['layout']>[number]
      | undefined
  }
  ...
  const heroCtaLabel =
    oc?.onlineCoursesHeroCtaLabel ?? (locale === 'de' ? 'Zu den Workshops' : 'View Workshops')
  const heroCtaUrl = oc?.onlineCoursesHeroCtaUrl ?? '/workshops'
  const heroCta2Label =
    oc?.onlineCoursesHeroCta2Label ?? (locale === 'de' ? 'Kontakt aufnehmen' : 'Contact us')
  const heroCta2Url = oc?.onlineCoursesHeroCta2Url ?? '/contact'
  const heroImageBreadResolved = resolveMedia(
    oc?.onlineCoursesHeroImageBread ?? oc?.onlineCoursesHeroImage,
  )
  const heroImageVegResolved = resolveMedia(
    oc?.onlineCoursesHeroImageVeg ?? oc?.onlineCoursesHeroImage,
  )
  const heroImageKimchiResolved = resolveMedia(
    oc?.onlineCoursesHeroImageKimchi ?? oc?.onlineCoursesHeroImage,
  )
  ...
  const learnCards =
    learnCardsRaw.length > 0
      ? learnCardsRaw
      : DEFAULT_LEARN_CARDS.map((c) => ({ title: c.title, description: c.description }))

  const modulesEyebrow =
    oc?.onlineCoursesModulesEyebrow ?? (locale === 'de' ? 'Kursübersicht' : 'Course Overview')
  ...
  const teamHeading = oc?.onlineCoursesTeamHeading
  const teamDescription = oc?.onlineCoursesTeamDescription
  const instructorStats = oc?.onlineCoursesInstructorStats
  const teamImagesRaw = (oc?.onlineCoursesTeamImages ?? []) as Array<{ image?: unknown }>
  const teamImages = teamImagesRaw
    .map((t) => resolveMedia(t.image))
    .filter((img): img is MediaType => Boolean(img))
  const teamButtonLabel = oc?.onlineCoursesTeamButtonLabel
  const teamButtonUrl = oc?.onlineCoursesTeamButtonUrl ?? '/about'
  const hasCoursesTeam = Boolean(teamHeading || teamDescription || teamImages.length)
  ...
  const howStepsRaw = (oc?.onlineCoursesHowSteps ?? []) as Array<{
    id?: string
    title?: string
    description?: string
  }>
  ...
  return (
    <article className="font-sans pb-0">
          ...
          <div className="grid min-h-[50vh] items-center gap-12 lg:grid-cols-2 lg:gap-16">
            <FadeIn>
              <div className="max-w-xl">
                ...
                {heroCtaHint && <p className="mt-3 text-body text-ff-gray-text">{heroCtaHint}</p>}
              </div>
            </FadeIn>
            <FadeIn delay={100}>
                  ...
                  <div className="absolute left-0 bottom-4 z-10 w-4/5 max-w-sm origin-bottom-left -rotate-4 overflow-hidden rounded-3xl bg-white shadow-2xl transition-transform duration-500 ease-out hover:-translate-y-1 hover:rotate-1">
                    <div className="relative aspect-4/3">
                      ...
                      <img
                        ...
                        alt={
                          typeof heroImageBreadResolved?.alt === 'string'
                            ? heroImageBreadResolved.alt
                            : 'Sourdough bread'
                        }
                  ...
                  <div className="absolute left-10 top-6 z-30 w-4/5 max-w-sm origin-center rotate-1 overflow-hidden rounded-3xl bg-white shadow-2xl transition-transform duration-500 ease-out hover:-translate-y-1.5 hover:scale-[1.03] lg:left-16 lg:top-10">
                    <div className="relative aspect-4/3">
                      ...
                      <img
                        ...
                        alt={
                          typeof heroImageVegResolved?.alt === 'string'
                            ? heroImageVegResolved.alt
                            : 'Vegetables in jar'
                        }
                        className="size-full object-cover"
                        fetchPriority="high"
                  ...
                  <div className="absolute right-0 top-2 z-20 w-3/4 max-w-sm origin-top-right rotate-5 overflow-hidden rounded-3xl bg-white shadow-2xl transition-transform duration-500 ease-out hover:-translate-y-1 hover:-rotate-1">
                    <div className="relative aspect-4/3">
                      ...
                      <img
                        ...
                        alt={
                          typeof heroImageKimchiResolved?.alt === 'string'
                            ? heroImageKimchiResolved.alt
                            : 'Kimchi'
                        }
                        className="size-full object-cover"
                        fetchPriority="high"
      ...
      <ContentSection bg="white" padding="sm" className="pt-6! md:pt-8!">
        <FadeIn delay={100}>
          <div className="relative mx-auto max-w-3xl text-center">
            ...
            <div className="mx-auto mt-3 h-1 w-16 rounded-full bg-ff-gold-accent" aria-hidden />
            {basicCoursePrice && (
              <div className="pointer-events-none absolute right-0 top-1/2 flex -translate-y-1/2 items-center justify-center px-0">
                <span className="pointer-events-auto inline-flex items-center rounded-full bg-ff-gold-accent px-5 py-2 font-display text-[1.75rem] font-bold leading-none text-ff-near-black shadow-sm">
                  {basicCoursePrice}
                </span>
              </div>
            )}
          ...
          <FadeIn delay={150}>
            ...
            >
              {workshopCards.map((card, i) => {
                ...
                const resolvedProduct = product && isResolvedProduct(product) ? product : undefined
      ...
      {hasCoursesTeam ? (
        <ContentSection bg="white" padding="sm">
          <FadeIn delay={80}>
            <div className="mx-auto max-w-5xl grid gap-10 lg:grid-cols-[minmax(0,2fr)_minmax(0,3fr)] items-start">
              <div className="space-y-3">
                ...
                {teamDescription && (
                  <p className="mt-2 text-body-lg leading-relaxed text-ff-gray-text">
                    {teamDescription}
                  </p>
                )}
                {instructorStats && (
                  <p className="mt-3 text-body-sm font-semibold text-ff-gray-text">
                    {instructorStats}
                  </p>
                )}
                {teamButtonLabel && teamButtonUrl && (
                  <Link
                    href={teamButtonUrl}
                    className="mt-4 inline-flex items-center justify-center rounded-full bg-ff-near-black px-6 py-2.5 font-display text-body font-bold text-white shadow-sm transition hover:bg-ff-charcoal"
                  >
                    {teamButtonLabel}
                  </Link>
                )}
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                {teamImages.map((img, i) => (
                  <FadeIn key={img.id ?? i} delay={120 + i * 80} from="bottom">
                    <div className="relative aspect-3/4 overflow-hidden rounded-2xl bg-ff-warm-gray">
                      <Media resource={img} className="size-full object-cover" priority={i === 0} />
                    </div>
                  </FadeIn>
                ))}
              </div>
      ...
      {/* 6. Testimonials — Was IHNEN AN UNSEREM Fermentationskurs gefällt — from Global */}
