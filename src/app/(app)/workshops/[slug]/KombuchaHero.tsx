...
/* ═══════════════════════════════════════════════════════════════
 *  KombuchaHero — Dedicated hero for /workshops/kombucha
 *
 *  Full-viewport split layout inspired by the home HeroSlider:
 *  LEFT:  Clustered glass jar silhouettes (placeholder)
 *  RIGHT: Eyebrow + title + description + CTA
 *
 *  Design language: cool blues/teals, clean typography,
 *  serif-like display font, award-worthy minimalism.
 * ═══════════════════════════════════════════════════════════════ */

// ─── Fermentation Glass Silhouette Placeholder ───────────────

function GlassJarSilhouette({ className, delay }: { className?: string; delay: number }) {
  return (
    <div
      className={`relative overflow-hidden rounded-2xl bg-linear-to-b from-[#b8d4e0]/50 to-[#9fc9d9]/40 shadow-xl backdrop-blur-sm transition-all duration-1000 ${className}`}
        ...
        <div className="relative h-[58vh] min-h-56 w-full overflow-hidden">
          {heroImage ? (
            ...
            <div className="flex h-full items-end justify-center pb-8 pt-28">
              ...
              >
                <GlassJarSilhouette className="h-32 w-24 -rotate-6" delay={200} />
                <GlassJarSilhouette className="h-40 w-28 rotate-2" delay={350} />
                <GlassJarSilhouette className="h-28 w-20 rotate-6" delay={500} />
              </div>
            </div>
          )}
        </div>
        ...
        <div className="relative flex-1 overflow-hidden">
          {heroImage ? (
            ...
            <div className="flex h-full items-end justify-center pb-16">
              ...
              >
                <GlassJarSilhouette className="h-44 w-32 -rotate-6 xl:h-52 xl:w-40" delay={300} />
                <GlassJarSilhouette className="h-56 w-40 rotate-2 xl:h-64 xl:w-48" delay={450} />
                <GlassJarSilhouette className="h-40 w-28 rotate-6 xl:h-48 xl:w-36" delay={600} />
              </div>
            </div>
          )}
        </div>
