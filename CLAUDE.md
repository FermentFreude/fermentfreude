...
Beyond the always-available `/project:fix`, `/project:debug`, `/project:new-feature`, `/project:new-block`, `/project:seed`, `/project:admin-cleanup`, `/project:promote-to-prod` — use these at the right point in a feature's lifecycle:
...
- **No script that can delete, mass-update, or reset data may ever run with production credentials — this is a hard rule, not a case-by-case judgment call.** The "Seeding Production" workflow documented above is for *additive* seeding only — it never applies to cleanup/reset/delete scripts. Any one-off production script, present or future, must check `process.env.DATABASE_URL` at the top and `process.exit(1)` with a loud error unless it recognizably points at the intended database (see `src/scripts/cleanup-test-orders.ts` / `cleanup-test-data.ts` for the pattern) — never just a code comment saying "don't run this on prod."
- **A Server Action or API route is reachable by anyone who can send it a request, regardless of which page renders the button that calls it.** Never rely on "the page that calls this is admin-gated" as the only protection for a mutating action — check the caller's role explicitly at the top of the action itself (see `requireAdmin()` in `src/components/admin/roster/actions.ts`). This applies to every future Roster/admin action, not just the ones that exist today.
- **Password strength has no config knob in Payload** — its `password` field validator defaults to a 3-character minimum with no `auth.minLength` option to raise it. `Users` collection enforces a real minimum (currently 8 chars) via a `beforeOperation` hook (`src/collections/Users/hooks/enforcePasswordStrength.ts`), since that's the only hook point that sees the plaintext password before hashing across create/update/reset. Don't remove this hook when touching auth — removing it silently reverts to the 3-character default.
- **Brute-force protection on login is `auth.maxLoginAttempts` + `lockTime` on `Users`, not a rate-limit middleware** — there is no rate-limiting library in this project. This is a deliberate choice: Payload persists lockout state per-user in MongoDB, so it holds across Vercel's serverless instances, whereas an in-memory limiter wouldn't. There is currently no comparable protection against `/api/users/forgot-password` being spammed for a given email (email-bombing / Brevo quota abuse) — flagged as a known gap, not yet fixed.
- **Any Payload collection whose `access` block returns `true` unconditionally for `create`/`update`/`delete` is a real vulnerability, not a convenience** — it means an unauthenticated request can write to it via Payload's REST/GraphQL API regardless of what the admin UI's own page-level gating implies. Use `adminOnly` (or the correct narrower access function) explicitly, every time, even for collections that "should" only ever be touched from `/admin`.
...
- `docs/BRAND.md` — Portable brand reference (colors, fonts, tone) — paste into AI design-skill prompts
